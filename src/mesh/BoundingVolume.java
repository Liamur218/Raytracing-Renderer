...
public class BoundingVolume implements Iterable<Polygon> {
    ...
    public Vector getBoundingVolumeIntersection(Vector origin, Vector dir) {
        // <x, y, z> = <mx, my, mz> * t + <x0, y0, z0>
        // x = mx * t + x0  ->  t = (x - x0) / mx
        // y = my * t + y0  ->  t = (y - y0) / my
        // z = mz * t + z0  ->  t = (z - z0) / mz
        ...
        double tLow, tHigh;
        Vector closestIntersection = new Vector(Double.MAX_VALUE);
        Vector low, high;
        ...
        if (dir.z != 0) {
            tLow = (min.z - origin.z) / dir.z;
            tHigh = (max.z - origin.z) / dir.z;
            low = Vector.add(Vector.multiply(dir, tLow), origin);
            high = Vector.add(Vector.multiply(dir, tHigh), origin);
            if (Util.isPointInside2DBox(low.x, low.y, min.x, max.x, min.y, max.y)) {
                if (Vector.distanceBetween(origin, low) < Vector.distanceBetween(origin, closestIntersection)) {
                    closestIntersection.set(low);
                }
            }
            if (Util.isPointInside2DBox(high.x, high.y, min.x, max.x, min.y, max.y)) {
                if (Vector.distanceBetween(origin, high) < Vector.distanceBetween(origin, closestIntersection)) {
                    closestIntersection.set(high);
                }
            }
        }
        ...
        if (dir.x != 0) {
            ...
            low = Vector.add(Vector.multiply(dir, tLow), origin);
            high = Vector.add(Vector.multiply(dir, tHigh), origin);
            if (Util.isPointInside2DBox(low.y, low.z, min.y, max.y, min.z, max.z)) {
                if (Vector.distanceBetween(origin, low) < Vector.distanceBetween(origin, closestIntersection)) {
                    closestIntersection.set(low);
                }
            }
            if (Util.isPointInside2DBox(high.y, high.z, min.y, max.y, min.z, max.z)) {
                if (Vector.distanceBetween(origin, high) < Vector.distanceBetween(origin, closestIntersection)) {
                    closestIntersection.set(high);
                }
            }
        ...
        if (dir.y != 0) {
            ...
            low = Vector.add(Vector.multiply(dir, tLow), origin);
            high = Vector.add(Vector.multiply(dir, tHigh), origin);
            if (Util.isPointInside2DBox(low.x, low.z, min.x, max.x, min.z, max.z)) {
                if (Vector.distanceBetween(origin, low) < Vector.distanceBetween(origin, closestIntersection)) {
                    closestIntersection.set(low);
                }
            }
            if (Util.isPointInside2DBox(high.x, high.z, min.x, max.x, min.z, max.z)) {
                if (Vector.distanceBetween(origin, high) < Vector.distanceBetween(origin, closestIntersection)) {
                    closestIntersection.set(high);
                }
            }
